...
export default function Home() {
  ...
  const iniciarProcessamento = async () => {
    ...
    itens.forEach(item => {
      const nomeNormalizado = item.nome.trim().toLocaleUpperCase('pt-BR');
      const candidatos = resultados.filter(resultado => codigoTemporario(resultado.codigo) &&
        resultado.nome.trim().toLocaleUpperCase('pt-BR') === nomeNormalizado);
      const correspondencias = itens.filter(outro => outro.nome.trim().toLocaleUpperCase('pt-BR') === nomeNormalizado);
      if (candidatos.length === 1 && correspondencias.length === 1 && !porCodigo.has(item.codigo)) {
        const temporario = candidatos[0];
        porCodigo.delete(temporario.codigo);
        porCodigo.set(item.codigo, { ...temporario, codigo: item.codigo, nome: item.nome, revisado: false });
      }
    });
