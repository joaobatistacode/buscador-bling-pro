...
export async function enviarProduto(
  ...
  if (!produto.codigo.trim() || /^TEMP-\d+$/i.test(produto.codigo.trim())) {
    return { codigo: produto.codigo, enviado: false, erro: 'Informe o código/SKU real do Bling antes de enviar.' };
  }
