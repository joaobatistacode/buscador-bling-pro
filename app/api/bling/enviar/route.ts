...
export async function POST(request: Request) {
  ...
  let pedidoValidado: Record<string, unknown>;
  try {
    const valor: unknown = JSON.parse(new TextDecoder().decode(corpoPedido));
    if (!valor || typeof valor !== 'object' || Array.isArray(valor)) throw new Error('JSON inválido');
    pedidoValidado = valor as Record<string, unknown>;
    const codigo = String(pedidoValidado.codigo ?? '').trim();
    if (!codigo || /^TEMP-\d+$/i.test(codigo)) {
      return Response.json({ erro: 'Informe o código/SKU real do Bling. Códigos temporários não podem ser enviados.' }, { status: 400 });
    }
  } catch {
